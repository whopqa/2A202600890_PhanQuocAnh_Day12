...
# Support both: running as `python -m Lab_Assigment.app.main` (Docker/prod)
# and running directly inside 06-lab-complete (local dev)
try:
    from Lab_Assigment.app.auth import verify_api_key
    from Lab_Assigment.app.config import settings
    from Lab_Assigment.app.cost_guard import close_cost_guard, record_usage
    from Lab_Assigment.app.rate_limiter import check_rate_limit, close_rate_limiter
    from Lab_Assigment.utils.mock_llm import ask as _mock_ask
except ModuleNotFoundError:
    from app.auth import verify_api_key  # type: ignore[no-redef]
    from app.config import settings  # type: ignore[no-redef]
    from app.cost_guard import close_cost_guard, record_usage  # type: ignore[no-redef]
    from app.rate_limiter import check_rate_limit, close_rate_limiter  # type: ignore[no-redef]
    from utils.mock_llm import ask as _mock_ask  # type: ignore[no-redef]


def llm_ask(question: str) -> str:
    """Call LLM: delegate to RAG system if available, else use mock."""
    import os
    customer_url = os.getenv("DAY08_CUSTOMER_AGENT_URL", "")
    if customer_url:
        try:
            import asyncio, httpx  # noqa: E401
            async def _ask_rag() -> str:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    resp = await client.post(
                        f"{customer_url}/api/chat",
                        json={"question": question, "session_id": None},
                    )
                    resp.raise_for_status()
                    return resp.json().get("answer", "")
            return asyncio.run(_ask_rag())
        except Exception:
